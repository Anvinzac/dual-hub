...
import { getScreenshotsForAppId } from "@/features/business/data/thumbnails";
import { useRemixes } from "@/features/business/hooks/useApps";
import { BUSINESS_AI_GUIDE_PATH } from "@/features/business/routes";
...
const AppDetailSheet = ({
  ...
  const handleOwnIt = () => {
    ...
    setTimeout(() => navigate(BUSINESS_AI_GUIDE_PATH), 300);
  };
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      url: `${window.location.origin}/business?tool=${app.id}`,
    };
    try {
      if (navigator.share) {
  ...
  const appScreenshots = getScreenshotsForAppId(app.id);
